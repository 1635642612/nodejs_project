; (function ($) {
    $.fn.pageCityAddress = function (cityNameId, callback) {
        var This = this;
        This.cityNameId = "#" + cityNameId;
        This.searchCitySelectUC = null;
        This.searchCityTitleUC = null;
        This.searchCityListUC = null;
        This.searchCityJsonUC = null;
        This.cityPageUC = null;
        This.cityList = [];
        This.zIndex = 200;

        This._enabled = false;
        This.setTimeoutMilliseconds = 200;
        This.getSourceJsonTimeout = null;
        //This.baseUrl = document.getElementById("BaseUrl").value;
        //This.baseUrl = "https://trains.ctrip.com/TrainBooking";
        //This.baseUrl = "http://m.uat.ctripqa.com/restapi/soa2/16042/json/searchStationListByKey";
        This.curSearchName = "";
        This.pageSize = 12;
        This.pageIndex = 1;
        This.pageCount = 1;
        This.maxPageMa = 5;//最大分页码
        ...
        $(This).bind("focus", function (e) {
            ...
            setTimeout(function () {
                ...
                This._enabled = false;
                if (!$(This.searchCityListUC).is(":visible")) {
                    showHostTemplete(me);
                }
            })
            ...
            setTimeout(function () {
                ...
                if (!This._enabled) {
                    if ($(This.searchCityListUC).is(":visible")) {
                        //设置搜索默认值
                        setSearchDefaultValue($(This.cityNameId).val().trim());
                    }
                    $(This.searchCityListUC).hide();
                    $(This.searchCitySelectUC).hide();
                }
            })
        }).bind("keydown", function (e) {
            This.getSourceJsonTimeout && clearTimeout(This.getSourceJsonTimeout);
        }).bind("keyup", function (e) {
            var me = $(this);
            This.curSearchName = $(This.cityNameId).val().trim();

            //前端展示补偿
            if (!This.cityList
                || This.cityList.length == 0) {
                var titleHtml = This.curSearchName + "，按拼音排序";
                $(This.searchCityTitleUC).html(titleHtml);
            }

            if (This.curSearchName && This.curSearchName.length > 0) {
                showCityListTemplete(me);
                This.getSourceJsonTimeout = setTimeout(function () {
                    var cityName = $(This.cityNameId).val().trim();
                    if (This.curSearchName == cityName) {
                        getSourceJsonData(This.curSearchName);
                        showCityListTemplete(me);
                    }
                }, This.setTimeoutMilliseconds);
            } else {
                showHostTemplete(me);
            }
        ...
        function appendHost() {
                    ...
                    if (cityList && cityList.length > 0) {
                        ...
                        if (searchid == "HOST") {
                            ...
                            for (var i = 0; i < cityList.length; i++) {
                                ...
                                var cityName = res.data.split("|")[1];
                                var citypy = res.data.split("|")[2];
                                var spy = citypy[0].toLowerCase();//首字母
                                var cityItem = { "display": display, "cityName": cityName, "citypy": citypy };
                                if (!cjson[spy]
                                    || !cjson[spy].data
                                    || cjson[spy].data.length == 0) {
                                    cjson[spy] = new Object();
                                    cjson[spy].spy = spy;
                                    cjson[spy].data = [];
                                    cjson[spy].data.push(cityItem);
                                } else {
                                    cjson[spy].data.push(cityItem);
                                }
                            }
                            ...
                            if (cjson) {
                                html = "<div searchid=\"" + searchid + "\" class=\"city_item hide\" >";
                                for (var name in cjson) {
                                    var res = cjson[name];
                                    if (res && res.data && res.data.length > 0) {
                                        html += "<div class=\"city_item_in\" >";
                                        html += "<span class=\"city_item_letter\" >" + res.spy.toUpperCase() + "</span>";
                                        for (var k = 0; k < res.data.length; k++) {
                                            var display = res.data[k].display;
                                            var cityName = res.data[k].cityName;
                                            var citypy = res.data[k].citypy;
                                            html += "<a  class=\"host-city-item\" title=\"" + display + "\" href=\"javascript:void(0)\" cityname=\"" + cityName + "\"  citypy=\"" + citypy + "\"  >" + display + "</a>";
                                        }
                                        html += "</div>";
                                    }
                        ...
                        $(This.searchCitySelectUC).append(html);
            ...
            $(This.searchCitySelectUC).find(".host-city-item").bind("click", function (v) {
                var me = this;
                var display = $(me).attr("title");
                var cityname = $(me).attr("cityname");
                var citypy = $(me).attr("citypy");
                callback && callback(display, cityname, citypy);
                $(This.searchCitySelectUC).hide();
            });
        ...
        function appendCityList() {
            var searchCityListUCHtml = "<div class=\"search-city-select-uc-list\" style=\"display:none;\" >";
            searchCityListUCHtml += "</div>";
            var searchCityTitleHtml = " <span class=\"c-hd\" ></span>";
            //html += cName + "，按拼音排序";
            //html += "</span>";
            var cityJsonHtml = "<div class=\"city-uc-item-box\" ></div>";
            var cityPageHtml = "<div class=\"city-uc-page\" ></div>";

            This.searchCityListUC = $(searchCityListUCHtml);
            This.searchCityTitleUC = $(searchCityTitleHtml);
            This.searchCityJsonUC = $(cityJsonHtml);
            This.cityPageUC = $(cityPageHtml);

            $(This.searchCityListUC).append(This.searchCityTitleUC);
            $(This.searchCityListUC).append(This.searchCityJsonUC);
            $(This.searchCityListUC).append(This.cityPageUC);

            $("body").append($(This.searchCityListUC));

            $(This.searchCityListUC).bind("mousedown", function (e) {
                setTimeout(function () {
                    This._enabled = true;
                    $(This.searchCityListUC).show();
                    $(This.cityNameId).focus();
                });
            });

            $(This.searchCityListUC).bind("click", function (ev) {
                setTimeout(function () {
                    (function (ev) {
                        var ev = ev || window.event;
                        var target = ev.target || ev.srcElement;
                        var citem = $(target).closest(".city-uc-item");
                        if (citem && $(citem).attr("title")) {
                            var display = $(citem).attr("title");
                            var cityname = $(citem).attr("cityname");
                            var citypy = $(citem).attr("citypy");
                            callback && callback(display, cityname, citypy);
                            $(This.searchCityListUC).hide();
                        }
                    })(ev);
            ...
            $(This.cityPageUC).bind("click", function (ev) {
                var ev = ev || window.event;
                var target = ev.target || ev.srcElement;

                if ($(target).hasClass("pagenum")) {
                    var pageMa = $(target).attr("num");
                    if (!pageMa) return;
                    if (pageMa == "prev") {//上一页
                        This.pageIndex = This.pageIndex - 1;
                    } else if (pageMa == "next") {//下一页
                        This.pageIndex = This.pageIndex + 1;
                    } else {
                        This.pageIndex = parseInt(pageMa);
                    }
        ...
        function showHostTemplete(target) {
            var offset = getOffset(target);
            $(This.searchCitySelectUC).css("z-index", This.zIndex);
            $(This.searchCitySelectUC).css({ "top": offset.top, "left": offset.left });

            $(This.searchCityListUC).css("z-index", -1);
            $(This.searchCityListUC).hide();
            $(This.searchCitySelectUC).show();
        };

        function showCityListTemplete(target) {
            var offset = getOffset(target);
            $(This.searchCityListUC).css("z-index", This.zIndex);
            $(This.searchCityListUC).css({ "top": offset.top, "left": offset.left });

            $(This.searchCitySelectUC).css("z-index", -1);
            $(This.searchCitySelectUC).hide();
            $(This.searchCityListUC).show();
        }
        ...
        function getOffset(obj) {
            ...
            offset.top = $(obj).offset().top + $(obj)[0].offsetHeight;
            offset.left = $(obj).offset().left;
            return offset;
        };

        function changeHost(obj) {
            if (!$(obj).hasClass("hot_selected")) {
                $(This.searchCitySelectUC).find("ul li").removeClass("hot_selected");
                $(obj).addClass("hot_selected");
                bindDisplay();
            }
        };

        function bindDisplay() {
            var selectd = $(This.searchCitySelectUC).find("ul li.hot_selected")[0];
            var index = $(This.searchCitySelectUC).find("ul li").index(selectd);
            $(This.searchCitySelectUC).find("div.city_item").hide();
            $(This.searchCitySelectUC).find("div.city_item").eq(index).show();
        };

        function getSourceJsonData(cityName) {
            $.ajax({
                url: "https://m.ctrip.com/restapi/soa2/16042/json/searchStationListByKey", //请求的url地址
                dataType: "json", //返回格式为json
                async: true,//请求是否异步，默认为异步，这也是ajax重要特性
                data: { "key": encodeURIComponent(encodeURIComponent(cityName)) }, //参数值
                type: "GET", //请求方式
                success: function (req) {
                    setSourceJsonData(req, cityName);
                },
                error: function (e) {
                    //请求出错处理
                    console.log(e);
        ...
        function setSourceJsonData(json, cName) {
            if (cName != This.curSearchName) {
                return;
            }
            ...
            if (!json
                || !json.isSuccessful
                || !json.stationList
                || json.stationList.length == 0) {
                //未获取到数据提示
                $(This.searchCityTitleUC).html("对不起,找不到：" + cName);
                return;
            }

            var titleHtml = cName + "，按拼音排序";

            if (cName != This.curSearchName) {
                return;
            }

            This.cityList = json.stationList;

            This.pageIndex = 1;

            if (This.cityList.length > This.pageSize) {
                This.pageCount = parseInt(This.cityList.length / This.pageSize) + (parseInt(This.cityList.length % This.pageSize) > 0 ? 1 : 0);
            } else {
                This.pageCount = 1;
            }
        ...
        //设置搜索默认值
        function setSearchDefaultValue(searchName) {
            if (This.cityList
                && This.cityList.length > 0) {
                var firstCity = This.cityList[0];
                if (firstCity.pinYin.indexOf(searchName) != -1
                    || firstCity.stationName.indexOf(searchName) != -1
                    || firstCity.shortPinYin.indexOf(searchName) != -1) {
                    callback && callback(firstCity.stationName, firstCity.stationName, firstCity.pinYin);
                    return;
                }
            }
        ...
        function bindPageMa() {
            var pageHtml = "";
            if (This.pageCount <= This.maxPageMa) {
                for (var i = 1; i <= This.pageCount; i++) {
                    if (i == This.pageIndex) {
                        pageHtml += "<span class=\"pagenum cur\"  num=\"" + i + "\" >" + i + "</span>";
                    } else {
                        pageHtml += "<span class=\"pagenum\"  num=\"" + i + "\" >" + i + "</span>";
                    }
                }
            } else {
                if (This.pageIndex <= 3) {
                    for (var k = 1; k <= 5; k++) {
                        if (k == This.pageIndex) {
                            pageHtml += "<span class=\"pagenum cur\"  num=\"" + k + "\" >" + k + "</span>";
                        } else {
                            pageHtml += "<span class=\"pagenum\" num=\"" + k + "\" >" + k + "</span>";
                        }
                    }
                    pageHtml += "<span class=\"pagenum\" num=\"next\"  >>></span>";//下一页

                } else {
                    pageHtml += "<span class=\"pagenum\" num=\"prev\"  ><<</span>";//上一页
                    var startIndex = This.pageIndex - 2;
                    var endIndex = This.pageIndex + 2;
                    ...
                    if (parseInt(This.pageIndex + 2) < This.pageCount) {
                        pageHtml += "<span class=\"pagenum\" num=\"next\"  >>></span>";//下一页
                    }
                }
            }

            $(This.cityPageUC).html(pageHtml);
        };

        function bindPageData() {
            var startIndex = parseInt((This.pageIndex - 1) * This.pageSize);
            var endIndex = parseInt(This.pageIndex * This.pageSize);
            var pageJson = This.cityList.slice(startIndex, endIndex);
            var html = "";
            if (pageJson && pageJson.length > 0) {
                for (var i = 0; i < pageJson.length; i++) {
                    var res = pageJson[i];
                    var display = res.stationName;
                    var cityName = res.stationName;
                    var citypy = res.pinYin;
                    html += "<div class=\"city-uc-item\" title=\"" + display + "\" cityname=\"" + cityName + "\"  citypy=\"" + citypy + "\" >";
                    html += " <span class=\"c-left\">" + display + "</span>";
                    html += " <span class=\"c-right\">" + citypy + "</span>";
                    html += "</div>";
                }
            }

            $(This.searchCityJsonUC).html(html);
